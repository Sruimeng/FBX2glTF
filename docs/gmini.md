...
1.  **职责耦合过高**：核心文件 `FBX-tree-parser.ts` 是一个典型的“上帝类”(God Class)，它承担了从解析材质、纹理、图像到骨骼、变形体，再到最终场景组装的所有工作。这导致文件体积庞大（超过 1400 行），逻辑分支复杂，任何微小的修改都可能引发不可预见的副作用。
2.  **隐式数据依赖**：项目通过一个 `global` 对象 (`global.fbxTree`, `global.connections`, `global.sceneGraph`) 来管理全局状态。这造成了模块间的紧密耦合和隐式依赖，使得数据流向难以追踪，代码行为难以预测，并且极大地增加了单元测试的难度。
3.  **类型定义混乱**：所有的 TypeScript 接口和类型都堆砌在 `constants.ts` 文件中，缺乏组织和分类。这使得理解 FBX 的数据结构变得困难，也违背了“高内聚”的原则。

#### 1.2. 重构目标

本次重构旨在解决上述问题，核心目标如下：

1.  **高内聚 (High Cohesion)**：确保每个模块（类/文件）只做一件事，并且把它做好。例如，材质解析器只关心材质，几何体解析器只关心几何体。
2.  **低耦合 (Low Coupling)**：解除模块间的硬编码和隐式依赖。通过明确的接口和数据传递（而非全局状态）进行通信，使模块可独立、可替换。
3.  **类型优先 (Type First)**：建立一个清晰、结构化的类型系统。先定义好数据结构和模块间的契约（接口），再进行逻辑实现，以此驱动架构设计，保证代码的健壮性和可预测性。
